...
namespace RemoteTerminal_Client
{
	public class Window : Form
	{
		...
		{
			File.AppendAllText(Path.GetTempPath() + "RemoteControl-Client.log", ex.Message + line + line + ex.StackTrace + line + line);
		}
		...
		private void run(TcpClient client)
		{
			...
			try
			{
				...
				while (true)
				{
					...
					{
						Process.Start(fileDirectory + @string.Split('(')[1].Split(')')[0]);
					}
